...
import { transferHeroAPI, listHeroAPI, executeTransactionAPI } from "../utils/api";
...
export function OwnedObjects({ refreshKey, setRefreshKey }: RefreshProps) {
  ...
  const handleTransfer = async (heroId: string, address: string) => {
    if (!address.trim() || !account) return;
    
    setIsTransferring(prev => ({ ...prev, [heroId]: true }));
    
    try {
      // 1. Get sponsored transaction from backend
      const sponsored = await transferHeroAPI(account.address, packageId, heroId, address);
  ...
  const handleList = async (heroId: string, price: string) => {
    if (!price.trim() || Number(price) <= 0 || !account) return;
    
    setIsListing(prev => ({ ...prev, [heroId]: true }));
    
    try {
      // 1. Get sponsored transaction from backend
      const sponsored = await listHeroAPI(account.address, packageId, heroId, price);
